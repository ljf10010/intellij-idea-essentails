...
当定义一个新的快捷键时，```Second Stroke```快捷键在编辑功能是非常有用的。你可以使用它(Second Stroke)替代冗余的快捷键甚至快捷键组，且方便你记住快捷键。例如， 你可以先定义一个基础的快捷键，```Ctrol+Shift+O```，然后使用一个数字作为它的```Second Stroke```，如下图所示：
