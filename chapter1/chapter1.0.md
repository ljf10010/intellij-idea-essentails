...
### 固定模式(Pinned Mode)
当工具窗口开启了<code>固定模式</code>后，那么它会一直显示在工作区中(不管是否处于激活状态)。就<code>Project</code>工具窗口来说，你可能更加喜欢一直显示它，以方便浏览项目的结构。固定模式依赖下面即将要出场的<code>停驻模式</code>。当然，你也可以选择一直关闭它，使用一些快捷键去查找，
