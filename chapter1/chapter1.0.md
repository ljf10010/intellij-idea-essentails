...
### 为项目设置个性化的工具窗口布局
在IntelliJ IDEA插件仓库中有一个非常有用的工具窗口管理插件。关于插件的更多的细节，我们将在稍后讲解。这个插件有什么牛逼的地方呢？默认情况下设置都是全局的，没有办法为每一个项目做个性化定制(eclipse 天生支持)。答案很明显了，这个插件就是肩负这样的使命。让你可以为每一个项目设置个性差异的工具窗口布局。
