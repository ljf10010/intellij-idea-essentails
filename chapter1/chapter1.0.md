...
当定义一个新的快捷键时，```Second Stroke```快捷键在编辑时非常有用的。你可以使用它(Second Stroke)替代冗余的快捷键甚至快捷键组，且方便你记住快捷键。例如， 你可以先定义一个基础的快捷键，```Ctrol+Shift+O```，然后使用一个数字作为它的```Second Stroke```，如下图所示：

![](1.13.png)

快捷键编辑器里的```Abbreviation选项```被用于快速全局查找(双击Shift)。关于快速全局查找的更多细节，将会在第三章-编辑器中讨论。
### 颜色与字体
在IntelliJ IDEA里，你可以根据你的喜好，随意调整编辑器中的语法或错误的高亮、搜索结果、调试器、控制台的字体布局和颜色。为此，打开IDE设置对话框中的```Editor|Colors & Fonts```部分。
### 挑选你的插件(Plugins)
